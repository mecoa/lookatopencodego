#!/usr/bin/env python3
"""从官方文档抓取并更新 OpenCode Go 政策信息到 config.json。

用法:
    python3 scripts/update_policy.py            # 预览（不落盘）
    python3 scripts/update_policy.py --apply    # 写回 config.json（自动备份）
...
def main():
    ...
    ap.add_argument("--apply", action="store_true", help="写回 config.json（默认仅预览）")
    ap.add_argument("--json", action="store_true", help="输出原始 JSON")
    args = ap.parse_args()

    cfg = Config(args.config)
    result = policy.refresh_policy(cfg, args.config, dry_run=not args.apply)
    ...
    if result.get("dry_run"):
        print("\n[预览模式] 未写回。加 --apply 以应用（将备份 config.json）。")
    else:
        write = result.get("write", {})
        print("\n已写回 config.json（备份: %s）" % write.get("backup"))
